...
import {useFeaturedBusiness} from '../Queries/useBusinessList';

interface IFeaturedBusinessProps {
  navigation: IItem;
  onSelect?: (item: IBusinessItem) => void;
}

function FeaturedBusiness(props: IFeaturedBusinessProps) {
  const {navigation, onSelect} = props;
  const theme = useAppTheme();
  const {
    data: businessTabList = [],
    isLoading,
    refetch,
  } = useFeaturedBusiness();
  ...
  const keyExtractor = React.useCallback(
    (item: IBusinessItem, index: number) =>
      `key-featured-business-${index}-${item.documentId}-${item.isFollowing}`,
    [],
  );
...
const styles = StyleSheet.create({
  ...
  column: {width: '50%', marginVertical: 5},
});
